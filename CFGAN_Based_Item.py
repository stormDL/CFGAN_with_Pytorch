...
class Train_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        '''
        :return: item_condition, item_info, mask_matrix, target_matrix
        '''
        if idx == 0:
            idx = 1
        item_condition = self.purchase_matrix[idx]
        item_info = self.purchase_matrix[idx]
...
def train(dataset, data_loader, u_items, device, batch_size=32, epoches=10, g_lr=0.0001, d_lr=0.0001, g_steps=1, d_steps=1,
    ...
    # 设置模型相关的参数
    #--------------------------------------
    gen = G(user_nb, user_nb).to(device)
    dis = D(user_nb*2, user_nb).to(device)
    ...
    for e in range(epoches):
        ...
        for d_step in range(d_steps):
            for item_condition, item_info, mask_matrix, target_matrix in data_loader:
                predict_matrix = gen(item_info)

                fake_item_data = torch.cat([item_condition, predict_matrix*mask_matrix], dim=1)
                real_item_data = torch.cat([item_condition, item_info], dim=1)
                fake_prob = dis(fake_item_data)
                real_prob = dis(real_item_data)
        ...
        for g_step in range(g_steps):
            for idx, (item_condition, item_info, mask_matrix, target_matrix) in enumerate(data_loader):
                predict_matrix = gen(item_info)
                item_data = torch.cat([item_condition, predict_matrix*mask_matrix], dim=1)
                predict_prob = dis(item_data)

                loss = bce_loss(predict_prob, real_label[:len(predict_matrix)]) + 1*mse_loss(predict_matrix*mask_matrix, target_matrix)
                g_opt.zero_grad()
                loss.backward()
                g_opt.step()
                print('-' * 48, e*batch_size+idx, '-' * 48)
...
def test(gen_model, dataset, u_items, k=5):
    ...
    with torch.no_grad():
        for uid in u_items.keys():
            item_condition, item_info, mask_matrix, target_matrix = dataset[uid]
            # item_data = torch.cat([item_condition, item_info], dim=-1)
            predict_result = gen_model(item_info).cpu().numpy().tolist()
            tmp_list = []
...
if __name__ == '__main__':
    ...
    dataset = Train_Dataset(item_purchase_matrix, mask_nb=64, device=device)
